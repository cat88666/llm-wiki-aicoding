...
- 结构复杂度也有行动卡：[结构梳理行动卡](materials/04_结构梳理行动卡.md)
...
| §2 | 让 AI 只读老代码，交出结构理解 | [架构梳理行动卡](materials/01_架构梳理行动卡.md) |
| §3 | 把现状写成 spec，用特征测试锁住 | [特征测试行动卡](materials/02_特征测试行动卡.md) |
| §4 | 小步改，每步跑测试，故意改坏一次看红灯 | [安全重构执行卡](materials/03_安全重构执行卡.md) |
| §5 | 另一种难：代码拆了模块但逻辑还缠在一起 | [结构梳理行动卡](materials/04_结构梳理行动卡.md) |
| §6 | 把 Git 当成质量控制轨道，不只是版本历史 | — |
| §7 | 让 spec 活起来，别让它变成过期文档 | [spec ↔ issue 治理行动卡](materials/05_spec_issue治理行动卡.md) |
| §8 | 把 lint + pytest 设成 GitHub 门禁 | [GitHub AI 管线行动卡](materials/06_GitHub_AI管线行动卡.md) |
| §9 | 收成一个飞轮：生成 → 验证 → 修正 | [质量飞轮行动卡](materials/07_质量飞轮行动卡.md) |
...
输入代码：[code/01_read_legacy_structure/god_file_order_system/order_system.py](code/01_read_legacy_structure/god_file_order_system/order_system.py)。
...
配 [架构梳理行动卡](materials/01_架构梳理行动卡.md)。一套操作规程：
...
![安全重构四步：锁行为→审测试→小步改→守门禁；两条铁律](assets/02_safe_refactor.drawio.svg)

### 3.3 带走：特征测试行动卡

配 [特征测试行动卡](materials/02_特征测试行动卡.md)。核心就几条：
...
![绞杀者模式：新实现在旧系统外围逐步长出、逐步接管，全程不停机、每步可切回](assets/01_strangler.drawio.svg)
...
配 [安全重构执行卡](materials/03_安全重构执行卡.md)。执行纪律：
...
配 [结构梳理行动卡](materials/04_结构梳理行动卡.md)。和 §2 的架构梳理不同——§2 面对的是单文件上帝类，主要拆职责；这里面对的是多文件耦合，核心输出是依赖图和状态地图。
...
配 [spec ↔ issue 治理行动卡](materials/05_spec_issue治理行动卡.md)。一条核心铁律：
...
![一条回路：issue→分支→PR→CI 门禁→人审→合并](assets/03_github_pipeline.drawio.svg)
...
打开 [code/05_github_pipeline/.github/workflows/ci.yml](code/05_github_pipeline/.github/workflows/ci.yml)，看这条最小 workflow：
...
配 [GitHub AI 管线行动卡](materials/06_GitHub_AI管线行动卡.md)。核心是把 AI 的产出变成工程对象：
...
![两个案例同一个飞轮：生成→验证（特征测试 / CI 门禁）→修正→人最终决策](assets/04_quality_flywheel.drawio.svg)
