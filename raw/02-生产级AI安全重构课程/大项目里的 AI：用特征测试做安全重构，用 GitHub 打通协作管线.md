...
```python
# 运行遗留系统，确认它可以正常执行
# !cd code && python 01_read_legacy_structure/god_file_order_system/order_system.py
```
...
输入代码：[code/01_read_legacy_structure/advanced_modular_coupling/](code/01_read_legacy_structure/advanced_modular_coupling/)。
...
这套结构在 [code/04_live_spec_template/](code/04_live_spec_template/)：
...
输入代码：[code/05_github_pipeline/](code/05_github_pipeline/)。
...
# !cd code/05_github_pipeline && pip install -e ".[dev]" && ruff check src tests && pytest -q
```
