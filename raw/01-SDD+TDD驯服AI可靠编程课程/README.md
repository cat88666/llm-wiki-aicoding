...
1. **讲义**：[用 SDD + TDD 驯服 AI：让代码既能「写得快」也能「信得过」.ipynb](用 SDD + TDD 驯服 AI：让代码既能「写得快」也能「信得过」.ipynb)——学员直接阅读的课程正文（VS Code Jupyter 打开）。标 🔴 的环节，现场从规格长出代码。
...
```
用 SDD + TDD 驯服 AI：让代码既能「写得快」也能「信得过」.ipynb
                               学员阅读版（19 cell）
code/
  演示脚本.md                   现场生成编排
  backend/
...
- 提案（节奏/时间轴/必讲清单）：原始 `提纲提案/客座1_大厂工程师的AI-Coding方法与经验_提纲.md`，当前仓库未包含
- 资料出处：原始 `资料调研/`，当前仓库未包含
